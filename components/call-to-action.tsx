...
export function CallToAction({ whatsappLink, title, description, buttonText, scheduleText }: Readonly<CallToActionProps>) {
  return (
    <section className="bg-amber-800 py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="container px-4 md:px-6 relative z-10">
        <div className="flex flex-col items-center space-y-6 text-center">
          <h2 className="text-3xl font-bold tracking-tight text-amber-50 sm:text-4xl max-w-2xl">{title}</h2>
          <p className="mx-auto max-w-[600px] text-amber-100/90 md:text-xl leading-relaxed">{description}</p>
          <Link
            href={whatsappLink}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-6 inline-flex items-center justify-center rounded-full bg-green-600 px-8 py-4 text-base font-medium text-white shadow-lg transition-all duration-300 hover:bg-green-700 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 group"
          >
            <svg
